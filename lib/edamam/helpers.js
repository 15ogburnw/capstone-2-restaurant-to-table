...
export function makeURL(baseURL, queryParams) {
  ...
  newParams.q = queryParams.q;
  console.log("helpers::queryParams", queryParams);
  console.log("helpers::newParams", newParams);
  ...
  for (let type of dishType) {
    type = replaceSpaces(type);
    url += `dishType=${type}&`;
  }
  for (let type of cuisineType) {
    type = replaceSpaces(type);
    url += `cuisineType=${type}&`;
  }
  for (let label of diet) {
    label = replaceSpaces(label);
    url += `diet=${label}&`;
  }
  for (let label of health) {
    label = replaceSpaces(label);
    url += `health=${label}&`;
  }
  ...
  console.log("helpers::finalURL", url);
  return url;
}
