...
export function truncateRecipe(data) {
  ...
  if (recipe.cuisineType) {
    for (let value of CUISINE_TYPES) {
      if (value === recipe.cuisineType.join()) {
        cuisineType = type;
      }
    }
  }
...
export const recipePagination = async (type, results) => {
  ...
  // If we don't have the recipes yet, get them from edamam
    ...
    console.log("nextpageurl before call in pagination", nextPageURL);
